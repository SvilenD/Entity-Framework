namespace BookShop.Initializer.Generators
{
    ...
    internal class BookGenerator
    {
        ...
        internal static Book[] CreateBooks()
        {
            ...
            {
                string[] booksData = booksInput[i].Split();

                int edition = int.Parse(booksData[0]);

                DateTime releaseDate = DateTime.ParseExact(booksData[1], "d/M/yyyy", CultureInfo.InvariantCulture);

                int copies = int.Parse(booksData[2]);

                decimal price = decimal.Parse(booksData[3]);

                int ageRestriction = int.Parse(booksData[4]);

                string title = String.Join(" ", booksData, 5, booksData.Length - 5);
