...
CREATE OR ALTER PROCEDURE usp_AddMinion(@villainId INT, @minionId INT, @townName VARCHAR(50), @villainName VARCHAR(50), @minionName VARCHAR(50), @minionAge INT)
AS
	DECLARE @TownId INT;
	SET @TownId = (SELECT Id FROM Towns WHERE Name = @townName);
	IF(NOT EXISTS(SELECT Id FROM Towns WHERE Id = @TownId))
	BEGIN
...
EXEC dbo.usp_AddMinion 1, 2, 'Berlin', 'villainName', 'minionName', 10;
